...
- 每张服务器卡片右上角有一个 **Detail**。它会用紧凑的 nvitop 风格表格汇总
  该服务器全部 GPU，显示 GPU 编号、用户名、进程名称、运行时间和 GPU 显存，
  不显示 PID。
- 同一张 GPU 的多个进程会归入独立分组；固定宽度的 `TIME` 和 `MEM` 列在滚动时
  仍保持对齐。
- 左键点击菜单栏图标可以打开或收起面板。
...
- **立即刷新：** 点击面板右上角的圆形刷新按钮。
- **GPU 进程详情：** 点击服务器卡片右上角的 **Detail**。详情直接使用最近一次
  刷新已取得的数据，不会额外建立 SSH 连接。
